...
PASSED_TEST=0
TOTAL_TEST=0
TOTAL_TEST_CASES=$(cat increment_test_cases.txt | wc -l)
for CURRENT_TEST_CASE_NUM in $(seq $TOTAL_TEST_CASES)
do
  CURRENT_TEST_CASE=$(cat increment_test_cases.txt | head -n$CURRENT_TEST_CASE_NUM | tail -n1)
